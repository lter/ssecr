...
- <u>Identify</u> some of the primary types of join
:::
...
library(tidyverse); library(supportR)
```
```{r diff-check-1}
# Create a dataframe of fish counts
fish_ct <- data.frame("species" = c("salmon", "bass", "halibut", "moray eel"),
                      "count" = c(12, 14, 23, 1))

# Create a fish taxonomy dataframe that corresponds with the other one
...
```{r diff-check-2}
# Use text replacement methods to fix the mistmatch between the two tables
...
### {{<fa rocket >}} Activity: Practice Joining Data
...
1.  Create a data frame where you bin months into seasons (i.e., winter, spring, summer, fall)
    -   Use your judgment on which month(s) should fall into each given PIE's latitude/location
