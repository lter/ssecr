...
- <u>Use</u> functions from the `tidyr` package to separate or combine columns
...
# Check structure
dplyr::glimpse(pie_crab)
```

Let's demonstrate this by splitting the 'date' column into its component pieces.

```{r separate-wider-delim}
# Split date into each piece of temporal info
pie_crab_spl <- pie_crab %>% 
