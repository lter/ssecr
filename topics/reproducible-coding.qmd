...
Doing your data operations with scripts is more reproducible than doing those operations without a programming language (i.e., with Microsoft Excel, Google Sheets, etc.). However, scripts are often written in a way that is not reproducible. A recent study aiming to run 2,000 projects' worth of R code found that _74% of the associated R files failed to complete without error_ (Trisovic _et al._ 2022). Many of those errors involve coding practices that hinder reproducibility but are easily preventable by the original code authors.
...
**When your scripts are reproducibly-written you will reap significant benefits!** These include:

1. Easier to return to your code after having set it down for weeks/months
2. Less verbal explanation needed when collaborating or onboarding new team members
3. Simpler to sharing methods for external result validation
4. In cases where you're developing a novel method or workflow, structuring your code reproducibly will increase the odds that someone outside of your team will adopt your strategy
...
![](topics/images/image_data-stages.png){fig-alt="Diagram depicting how raw data is transformed to cleaned data, then standardized data, and finally to published data products by a set of scripts between each 'type' of data" .lightbox}
...
**Consider whether there are privacy concerns with sharing your data with AI _before_ allowing an AI agent access to your local Git repository!** Especially if you have a "data/" folder in your Git repo becaause granting AI access to your Git folder will also include sub-directories of that folder (whether or not Git tracks them!)
