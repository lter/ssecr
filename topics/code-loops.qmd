...
# Let's loop across study sites
for(focal_site in unique(pie_crab$site)){ # <1>
  ...
  focal_avg <- mean(focal_df$size, na.rm = T) 
  focal_dev <- sd(focal_df$size, na.rm = T) 
  
  # Assemble this into a data table and add to our list
  crab_list[[focal_site]] <- data.frame("site" = focal_site,
                                        "crab.size_mean" = focal_avg,
                                        "crab.size_sd" = focal_dev)
} # Close loop
